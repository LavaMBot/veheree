...
client.on("message", message => {
 if (message.content === "-help") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter(' v9 gaming هذا البوت براعه سيرفر ')
	  ...
	  .addField('-kick', `لطرد عضو مع ذكر السبب`)
	  .addField('-ban', `مع ذكر السبب `) 
	  .addField('-mute', `هذا الامر يمنع العضو من الكتابه `) 
	  .addField('-unmute', `يستطيع العضو الكتابه مره اخري `) 
	  .addField('-role', `لاعطاء اي رتبه `) 
...
client.on("message", message => {
 if (message.content === "-invite") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('شكرا لاضافه البوت لسيرفارك')
      .addField('شكرا لك لاستخدامك ل بروميوم بوت', `https://discordapp.com/api/oauth2/authorize?client_id=544052801140031499&permissions=8&scope=bot`)
  message.author.send({embed});
...
client.on("message", message => {
 if (message.content === "-support") {
  const embed = new Discord.RichEmbed()
      ...
      .addField('سيرفر الدعم الفني', `  https://discord.gg/qnWANFk  `)
  message.author.send({embed});
